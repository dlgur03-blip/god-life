...
async function getDashboardStats(userId: string) {
  ...
  const destinyStatus = !destinyDay 
    ? { label: 'Not Started', color: 'text-gray-500' }
    : destinyDay.timeblocks.some(b => b.score && b.score > 0)
      ? { label: 'In Progress', color: 'text-primary' }
      : { label: 'Planned', color: 'text-blue-400' };
  ...
  const discStatus = discTotal === 0 
    ? { label: 'No Rules', color: 'text-gray-500' }
    : { label: `${Math.round((discChecked / discTotal) * 100)}% Done`, color: discChecked === discTotal ? 'text-green-400' : 'text-secondary' };

  const successCount = successProjects.length;
  const successStatus = successCount === 0
    ? { label: 'No Active Missions', color: 'text-gray-500' }
    : { label: `${successCount} Active`, color: 'text-accent' };
    
  const epistleStatus = epistleDay 
    ? { label: 'Sealed', color: 'text-purple-400' }
...
export default async function Home() {
    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center bg-[url('/bg-grid.svg')] text-center p-6">
        ...
        <p className="text-gray-400 mb-8 text-xl max-w-md">
          Architect your destiny. Master your discipline. Hack your biology.
        </p>
        <Link 
  ...
  const modules = [
    { name: 'Destiny Navigator', href: `/destiny/day/${todayStr}`, icon: Compass, desc: 'Daily Goals & Timeblocks', status: stats.destinyStatus },
    { name: 'Success Code', href: '/success', icon: Trophy, desc: '100 Days Projects', status: stats.successStatus },
    { name: 'Discipline Mastery', href: `/discipline/day/${todayStr}`, icon: Activity, desc: 'Daily Rules & Check', status: stats.discStatus },
    { name: 'Self Epistle', href: `/epistle/day/${todayStr}`, icon: Mail, desc: 'Letters to Self', status: stats.epistleStatus },
    { name: 'Bio Hacking', href: '/bio', icon: BookOpen, desc: 'Optimization Columns', status: { label: 'Database', color: 'text-green-400' } },
  ];
