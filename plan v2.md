...
##### 1.1.1 24시간 타임블록
- [ ] 00:00 ~ 23:59 전체 시간대 지원
- [ ] 기본값: 1시간 단위 블록 24개
- [ ] 사용자가 블록 시작/종료 시간 직접 수정 가능
- [ ] **5분 단위**로 시간 설정 가능 (예: 06:00 ~ 06:45)

##### 1.1.2 블록 관리 기능
- [ ] 블록 추가/삭제 기능
- [ ] 블록 시간 범위 드래그로 조절 (선택사항)
- [ ] 블록 순서 변경

##### 1.1.3 템플릿 저장 기능
- [ ] 현재 타임블록 구조를 "템플릿"으로 저장
- [ ] 템플릿 이름 지정 (예: "평일 루틴", "주말 루틴")
- [ ] 저장된 템플릿 불러오기
- [ ] 템플릿 삭제
...
#### 요구사항
- [ ] HeaderWrapper에 홈 버튼 추가
- [ ] 로고 클릭 시 대시보드(`/`)로 이동
- [ ] 또는 좌측 상단에 항상 홈 아이콘 표시
...
- [ ] 클라이언트에서 이미지 선택/촬영
- [ ] 이미지 리사이징 (최대 1920px)
- [ ] 클라우드 스토리지 업로드 (Vercel Blob 또는 Cloudinary)
- [ ] URL을 DB에 저장

##### 2.2.3 업로드된 이미지 표시
- [ ] 그리드에서 완료된 날짜 클릭 시 이미지 미리보기
- [ ] 이미지 확대 모달
...
##### 3.1.1 체크 가능 날짜 제한
- [ ] **오늘만** 체크 가능
- [ ] 과거 날짜: 읽기 전용 (체크 불가, 기록만 표시)
- [ ] 미래 날짜: 체크 불가 (잠금 표시)

##### 3.1.2 날짜 네비게이션
- [ ] 오늘 이전 날짜: 볼 수는 있지만 수정 불가
- [ ] 내일 이후 날짜: 접근은 가능하나 "아직 체크할 수 없습니다" 메시지
...
#### 구현 요구사항
- [ ] 오늘 페이지 진입 시 어제의 `toTomorrow` 데이터 조회하여 상단에 표시
- [ ] 무드 이모지로 어제 감정 상태 함께 표시
- [ ] 받은 서신 영역은 읽기 전용 (수정 불가)
- [ ] 작성 영역은 현재와 동일하게 유지
...
##### 5.1.1 다국어 콘텐츠 구조
- [ ] 각 칼럼별 ko, en, ja 버전 지원
- [ ] 사용자 locale에 따라 해당 언어 콘텐츠 표시
- [ ] 번역이 없는 경우 fallback (en → ko → ja)
...
- [ ] 각 언어별 콘텐츠 입력
- [ ] 미리보기 기능
- [ ] 카테고리 선택 (Nutrition, Supplements, Recovery, Exercise, Mindset 등)
...
#### 요구사항
- [ ] 특정 이메일 주소만 관리자로 지정 (환경변수 또는 DB)
- [ ] 관리자 아닌 사용자 접근 시 403 에러
- [ ] URL: `/admin` 또는 `/[locale]/admin`
